    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (stat.isDirectory()) {
          ...
          if (
            !(
              relativePath.includes('storybook-static') ||
...
async function main() {
  ...
  // Categorize files
  const phase2Files = fileViolations.filter(
    (f) => f.violations >= 20 && f.violations <= 60
  );
  ...
  console.log(
    `  High-priority files (>60 violations): ${highPriorityFiles.length}`
  );
  ...
  const formFiles = phase2Files.filter(
    (f) =>
