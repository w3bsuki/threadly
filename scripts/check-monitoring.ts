...
} from '../packages/observability/production-monitoring';
...
async function checkEnvironmentVariables() {
  ...
  try {
    ...
    if (env.SENTRY_DSN) {
      printSuccess(
        `Sentry DSN configured: ${env.SENTRY_DSN.substring(0, 30)}...`
      );
    } else {
...
async function main() {
  ...
  console.log(
    'This script validates your current monitoring setup and provides'
  );
