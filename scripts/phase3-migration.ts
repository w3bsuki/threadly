...
const auditData = JSON.parse(readFileSync('PHASE_3_AUDIT_RESULTS.json', 'utf-8'));
...
async function migratePhase3Files(): Promise<void> {
  ...
  for (let i = 0; i < auditData.files.length; i++) {
    ...
    // Show progress
    if (i % 20 === 0) {
      console.log(`📈 Progress: ${i}/${auditData.files.length} files processed`);
    }
  ...
  filesWithChanges
    ...
    .forEach((result, index) => {
      console.log(`${index + 1}. ${result.file} (${result.changesApplied} changes)`);
    });

  // Category breakdown
  const categories = {
    'design-system': filesWithChanges.filter(r => r.file.includes('packages/design-system')),
    'web-app': filesWithChanges.filter(r => r.file.includes('apps/web')),
    'mobile-app': filesWithChanges.filter(r => r.file.includes('apps/app')),
    'packages': filesWithChanges.filter(r => r.file.includes('packages/') && !r.file.includes('packages/design-system')),
  };
  ...
  writeFileSync('PHASE_3_MIGRATION_RESULTS.json', JSON.stringify(migrationSummary, null, 2));
