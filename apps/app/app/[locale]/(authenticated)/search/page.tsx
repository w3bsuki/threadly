...
const searchParamsSchema = z.object({
  q: z.string().optional().transform(val => val ? val.trim().replace(/[<>]/g, '') : undefined),
  ...
  sortBy: z.enum(['relevance', 'price_asc', 'price_desc', 'newest', 'most_viewed', 'most_favorited']).optional().default('relevance')
});
...
const SearchPage = async ({ params, searchParams }: SearchPageProperties) => {
  ...
  const { q, categories, brands, conditions, sizes, priceMin, priceMax, sortBy } = validatedSearchParams;
