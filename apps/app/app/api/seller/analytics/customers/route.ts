...
export async function GET(request: NextRequest) {
  try {
    ...
    const averageOrderValue =
      totalCustomers > 0 ? totalRevenue / orders.length : 0;
    ...
    const segments = [
      {
        name: 'High Spenders',
        count: spendingAmounts.filter((amount) => amount > avgSpending * 2)
          .length,
        percentage: 0,
        avgSpend:
          spendingAmounts
            .filter((amount) => amount > avgSpending * 2)
            .reduce((sum, amount) => sum + amount, 0) /
          Math.max(
            1,
            spendingAmounts.filter((amount) => amount > avgSpending * 2).length
          ),
        color: '#10b981',
      },
      {
        name: 'Medium Spenders',
        count: spendingAmounts.filter(
      ...
      },
      {
        ...
        avgSpend:
          spendingAmounts
            .filter((amount) => amount <= avgSpending)
            .reduce((sum, amount) => sum + amount, 0) /
          Math.max(
            1,
            spendingAmounts.filter((amount) => amount <= avgSpending).length
          ),
        color: '#f59e0b',
      },
    ];
    ...
    const topCustomers = Array.from(customerSpending.entries())
      .map(([buyerId, totalSpent]) => {
        ...
        const lastPurchase = Math.max(
          ...customerOrders.map((o) => o.createdAt.getTime())
        );
    ...
    const conversionRate =
      uniqueVisitors.size > 0 ? totalCustomers / uniqueVisitors.size : 0;
