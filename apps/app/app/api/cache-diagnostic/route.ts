      ...
      diagnostic: {
        ...
        recommendations: [
          !env.NEXT_PUBLIC_WEB_URL &&
            'NEXT_PUBLIC_WEB_URL is not set in environment variables',
          !env.ADMIN_SECRET &&
            'ADMIN_SECRET is not set in environment variables',
          !isHealthy &&
            `Cannot reach web app at ${webUrl}. Check if the URL is correct.`,
          testResponse.status === 401 &&
            'Authentication failed. Check if ADMIN_SECRET matches between apps.',
        ].filter(Boolean),
      },
    });
  } catch (error) {
