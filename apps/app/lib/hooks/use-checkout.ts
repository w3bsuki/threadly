...
export function useCheckout() {
  ...
  const calculateCosts = useCallback(() => {
    ...
    const subtotal = checkoutStore.session.items.reduce(
    ...
    try {
      ...
      if (
        !(
          ...
          checkoutStore.session.shippingAddress &&
          checkoutStore.session.paymentMethod
        )
      ) {
      ...
      const response = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          billingAddress:
            checkoutStore.session.billingAddress ||
            checkoutStore.session.shippingAddress,
