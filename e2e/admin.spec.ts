...
test.describe('Admin Panel', () => {
  ...
  test('should access admin dashboard', async ({ page }) => {
    ...
    const statsCards = page
      ...
      .or(page.locator('.card, [class*="card"]'));
  ...
  test('should manage products', async ({ page }) => {
    ...
    const productTable = page
      .getByRole('table')
      .or(page.locator('[data-testid="products-list"]'));
  ...
  test('should moderate product listings', async ({ page }) => {
    ...
    const actionButton = productRow
      .getByRole('button', { name: /action|moderate|approve|remove/i })
      .first();
  ...
  test('should manage users', async ({ page }) => {
    ...
    const userActionButton = page
      .getByRole('button', { name: /action|ban|suspend/i })
      .first();
  ...
  test('should handle bulk operations', async ({ page }) => {
    ...
    if (await productCheckboxes.first().isVisible()) {
      ...
      if (await bulkActionsBar.isVisible()) {
        ...
        const bulkRemoveButton = page.getByRole('button', {
          name: /remove|delete/i,
        });
        const bulkArchiveButton = page.getByRole('button', {
          name: /archive/i,
        });
  ...
  test('should show admin statistics and analytics', async ({ page }) => {
    ...
    const statsCards = page.locator(
      '[data-testid*="stat"], .stat, [class*="stat"]'
    );
    await expect(statsCards.first()).toBeVisible();
    ...
    const timeFilter = page
      .getByRole('button', { name: /week|month|year/i })
      .or(page.locator('select').filter({ hasText: /period|time/i }));

    if (await timeFilter.isVisible()) {
      await timeFilter.click();

      const timeOption = page
        .getByText(/last 7 days|last month|last year/i)
        .first();
