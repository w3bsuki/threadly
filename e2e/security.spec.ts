...
test.describe('Frontend Security Tests', () => {
  ...
  test('should validate file upload security', async ({ page }) => {
    ...
    if (await fileInput.isVisible()) {
      ...
      const errorMessage = page.getByText(
        /invalid file type|only images allowed/i
      );
  ...
  test('should validate URL parameters for injection', async ({ page }) => {
    ...
    await page.goto(
      '/search?q=' + encodeURIComponent('<script>alert("xss")</script>')
    );
  ...
  test('should handle error pages securely', async ({ page }) => {
    ...
    expect(pageContent).not.toMatch(
      /database|server|internal error|stack trace/i
    );
  ...
  test('should protect sensitive routes', async ({ page }) => {
    ...
    const isProtected =
      ...
      currentUrl.includes('login') ||
      (await page
        .getByText(/access denied|unauthorized|admin access required/i)
        .isVisible());
