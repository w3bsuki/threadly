...
export class SecurityHeaders {
  ...
  apply(response: NextResponse): NextResponse {
    ...
    if (this.config.strictTransportSecurity !== false) {
      headers.set('Strict-Transport-Security', this.config.strictTransportSecurity)
    }
    ...
    if (this.config.xPermittedCrossDomainPolicies !== false) {
      headers.set('X-Permitted-Cross-Domain-Policies', this.config.xPermittedCrossDomainPolicies)
    }
...
export const createSecurityMiddleware = (config: SecurityHeadersConfig = defaultSecurityHeaders) => {
  const security = new SecurityHeaders(config)
...
export const corsHeaders = (options: {
