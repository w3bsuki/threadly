...
export class ApiVersioning {
  ...
  private defaultVersionExtractor(req: NextRequest): ApiVersion | null {
    ...
    if (acceptHeader) {
      const versionMatch = acceptHeader.match(/application\/vnd\.threadly\.(\w+)\+json/)
      if (versionMatch) {
  ...
  private defaultDeprecationHandler(version: ApiVersion, req: NextRequest): void {
  ...
  createRouter<T extends Record<ApiVersion, any>>(handlers: T) {
    return async (req: NextRequest, params: any): Promise<NextResponse> => {
      ...
      if (!handler) {
        throw ApiError.badRequest(`Handler not implemented for API version ${version}`)
      }
...
export const withApiVersion = (
  handler: (req: NextRequest, params: any, version: ApiVersion) => Promise<NextResponse>,
...
export const createVersionedEndpoint = <T = any>(
  ...
  return async (req: NextRequest, params: T): Promise<NextResponse> => {
    ...
    if (!handler) {
      throw ApiError.badRequest(`Endpoint not available in API version ${version}`)
    }
