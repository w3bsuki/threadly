...
import type { ActionState, ServerActionResult } from '../types';
...
export function useOptimisticServerAction<
  TInput,
  TOutput,
  TOptimistic = TOutput,
>(
  ...
  options?: UseServerActionOptions & {
    optimisticUpdate?: (
      currentData: TOptimistic | undefined,
      input: TInput
    ) => TOptimistic;
  }
) {
...
export function useInfiniteServerAction<
  ...
  TOutput extends { nextCursor?: string | null },
>(
  action: (input: TInput) => Promise<ServerActionResult<TOutput>>,
  options?: UseServerActionOptions
) {
