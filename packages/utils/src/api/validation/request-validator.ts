...
export class RequestValidator {
  ...
  static validateParams<T>(params: unknown, schema: ZodSchema<T>): T {
    try {
  ...
  static validateHeaders<T>(request: NextRequest, schema: ZodSchema<T>): T {
    try {
...
export const createValidator = <T>(schema: ZodSchema<T>) => {
  return {
    body: (request: NextRequest) =>
      RequestValidator.validateBody(request, schema),
    query: (request: NextRequest) =>
      RequestValidator.validateQuery(request, schema),
    params: (params: unknown) =>
      RequestValidator.validateParams(params, schema),
    headers: (request: NextRequest) =>
      RequestValidator.validateHeaders(request, schema),
    formData: (request: NextRequest) =>
      RequestValidator.validateFormData(request, schema),
  };
};
