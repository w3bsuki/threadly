...
export function useSearch(options: UseSearchOptions = {}): UseSearchReturn {
  ...
  const search = useCallback(
    async (filters: SearchFilters, page = 0) => {
      ...
      try {
        ...
        if (filters.sellerRating)
          params.set('sellerRating', filters.sellerRating.toString());
  ...
  const updateSavedSearch = useCallback(
    async (id: string, updates: Partial<SavedSearch>): Promise<boolean> => {
      if (!user?.id) return false;
  ...
  const toggleSearchAlerts = useCallback(
    async (id: string, enabled: boolean): Promise<boolean> => {
      ...
      try {
        const response = await fetch(
          `/api/saved-searches/${id}/toggle-alerts`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
