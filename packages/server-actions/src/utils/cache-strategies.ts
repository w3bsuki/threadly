  ...
  product: {
    list: {
      ...
      tags: (filters?: any) => [
        ...
        ...(filters?.categoryId ? [cacheTags.byQuery('products', `category:${filters.categoryId}`)] : []),
        ...(filters?.status ? [cacheTags.byStatus('products', filters.status)] : []),
      ],
    },
    detail: {
...
export async function staleWhileRevalidate<T>(
  ...
  const { staleTime = 60000, cacheTime = 300000 } = options // 1 min stale, 5 min cache
...
async function getCachedValue<T>(key: string): Promise<{ data: T; timestamp: number } | null> {
