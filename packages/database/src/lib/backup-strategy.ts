...
export const backupStrategy: BackupConfig = {
  ...
  retention: {
    daily: parseInt(process.env.DATABASE_BACKUP_RETENTION_DAILY || '7'),
    weekly: parseInt(process.env.DATABASE_BACKUP_RETENTION_WEEKLY || '4'),
    monthly: parseInt(process.env.DATABASE_BACKUP_RETENTION_MONTHLY || '6'),
  },
  storage: {
  ...
  encryption: {
    ...
    algorithm: process.env.DATABASE_BACKUP_ENCRYPTION_ALGORITHM || 'aes-256-gcm',
  },
  notifications: {
...
export const getBackupCommand = (databaseUrl: string, outputPath: string): string => {
...
export const getRestoreCommand = (databaseUrl: string, backupPath: string): string => {
  const url = new URL(databaseUrl)
  const dbName = url.pathname.slice(1)
  const host = url.hostname
  const port = url.port || '5432'
