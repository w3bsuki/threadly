...
export class DatabaseMonitor {
    ...
    this.prisma.$use(async (params, next) => {
      ...
      try {
        ...
        this.recordMetric({
          ...
          timestamp: new Date(),
          params:
            process.env.NODE_ENV === 'development' ? params.args : undefined,
        });
  ...
  private startMetricsCleanup(): void {
    setInterval(() => {
      const cutoff = new Date(
        Date.now() - this.metricsRetentionMinutes * 60 * 1000
      );
  ...
  getMetricsSummary() {
    ...
    for (const metric of metrics) {
      ...
      byModel[modelKey].avgDuration =
        (byModel[modelKey].avgDuration * (byModel[modelKey].count - 1) +
          metric.duration) /
        byModel[modelKey].count;

      byAction[actionKey].count++;
      byAction[actionKey].avgDuration =
        (byAction[actionKey].avgDuration * (byAction[actionKey].count - 1) +
          metric.duration) /
        byAction[actionKey].count;
    }
    ...
    return {
      ...
      slowQueries: metrics.filter((m) => m.duration > this.slowQueryThreshold)
        .length,
      averageDuration:
  ...
  async getTableSizes(): Promise<
    Array<{ table: string; size: string; rows: string }>
  > {
    try {
      const result = await this.prisma.$queryRaw<
        Array<{ table_name: string; total_size: string; row_estimate: string }>
      >`
        SELECT 
  ...
  async getSlowQueries(
    limit = 10
  ): Promise<Array<{ query: string; calls: number; mean_time: number }>> {
    try {
      const result = await this.prisma.$queryRaw<
        Array<{ query: string; calls: string; mean_exec_time: number }>
      >`
        SELECT 
          query,
          calls::text,
