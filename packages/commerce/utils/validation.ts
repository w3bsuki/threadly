...
import { isValidPrice, type PriceInput, toNumber } from './price';
...
export function validateProductPrice(
  ...
  if (!isValidPrice(price)) {
    ...
    } else if (numericPrice > 999_999.99) {
  ...
  if (category && numericPrice > 0) {
    ...
    if (minPrice && numericPrice < minPrice) {
      warnings.push(
        `Price seems low for ${category} items. Consider pricing above $${minPrice}`
      );
    }
  }

  // Suspiciously high price warning
  if (numericPrice > 10_000) {
...
export function validateProductListing(product: {
  ...
  if (!product.title?.trim()) {
    ...
    warnings.push(
      'Product title should be at least 10 characters for better searchability'
    );
  ...
  if (!product.description?.trim()) {
    ...
    warnings.push(
      'Detailed descriptions help buyers make decisions. Consider adding more details'
    );
...
export function validateOrder(order: {
  ...
  if (order.shippingAddress) {
    ...
    if (!addr.postalCode?.trim()) {
      ...
      warnings.push(
        'Postal code format may be invalid for the selected country'
      );
    }
