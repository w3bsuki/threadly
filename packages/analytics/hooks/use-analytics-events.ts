...
export function useAnalyticsEvents() {
  ...
  const trackProductFavorite = useCallback(
    (product: any, isFavorited: boolean) => {
      if (!analytics) return;
  ...
  const trackProductQuickView = useCallback(
    (product: any) => {
      track(
        AnalyticsEvents.PRODUCT_QUICK_VIEW,
  ...
  const trackProductListed = useCallback(
    (product: any) => {
  ...
  const trackSearchQuery = useCallback(
    (query: string, resultsCount?: number, filters?: any) => {
      if (!analytics) return;
  ...
  const trackSearchFilters = useCallback(
    (filters: any) => {
  ...
  const trackLoadMore = useCallback(
    (contentType: string, currentCount: number) => {
  ...
  const trackProfileView = useCallback(
    (viewedUserId: string, viewerUserId?: string) => {
  ...
  const trackCategoryView = useCallback(
    (categoryName: string, productCount?: number) => {
